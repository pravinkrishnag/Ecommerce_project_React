...
export const getProductsbySeller = (id) => async (dispatch) => {
  ...
  try {
    const result = await axios.get(
      `${REACT_APP_BASE_URL}/getSellerProducts/${id}`
    );
    if (result.data.message) {
...
export const getProductDetails = (id) => async (dispatch) => {
  ...
  try {
    const result = await axios.get(
      `${REACT_APP_BASE_URL}/getProductDetail/${id}`
    );
    if (result.data.message) {
...
export const getSpecificProducts = (id, address) => async (dispatch) => {
  ...
  try {
    ...
    if (result.data.message) {
      ...
      console.log("Received data:", result.data); // Add this line for debugging
      dispatch(specificProductSuccess(result.data));
    }
  } catch (error) {
    console.error("Error in getSpecificProducts:", error); // Add this line for debugging
    dispatch(getError(error));
  }
};
